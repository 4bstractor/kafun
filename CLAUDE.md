...
| `.gitea/workflows/` + `.github/workflows/` | Split CI/release: the `.gitea/` copies run on Gitea Actions and publish to giyouden (`artifacts.harvelab.com`); the `.github/` copies run on GitHub Actions and publish to `ghcr.io`. Gitea only falls back to `.github/workflows/` when `.gitea/workflows/` is absent, so both remotes share one branch. A `v*` tag pushed to both remotes publishes both images. |
